...
def create_node(df, mode):
    ...
    elif mode == 2:  # constant sample feature [1], indexed feature embedding
        feature_ind = np.array(range(ncol))
        sample_node = [[1] for i in range(nrow)]
        node = sample_node + feature_ind.reshape((ncol, 1)).tolist()
    return node
...
def get_data(df_X, df_y, node_mode, train_edge_prob, split_sample_ratio, split_by, train_y_prob, seed=0, normalize=True, one_hot=False):
    if len(df_y.shape)==1:
        df_y = df_y.to_numpy()
    elif len(df_y.shape)==2:
        df_y = df_y[0].to_numpy()
    if one_hot: 
        df_y = np.eye(np.unique(df_y).shape[0])[df_y].astype(int)
...
def load_data(adata, target_col='', node_mode=0, train_edge=0.7, split_sample=0, split_by='y', train_y=0.7, **kwargs):
    df_y = pd.DataFrame(adata.obs[target_col].values)
    df_X = pd.DataFrame(adata.X)
    data = get_data(df_X, df_y, node_mode, train_edge, split_sample, split_by, train_y, **kwargs)
    return data
