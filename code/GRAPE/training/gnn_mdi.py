...
def train_gnn_mdi(data, args, log_path, device=torch.device('cpu')):
    ...
    if args.transfer_dir: # this ensures the valid mask is consistant
        load_path = './test/{}/'.format(args.transfer_dir)
    ...
    for epoch in range(args.optimizer.epochs):
        ...
        # print(x_embd.shape, x_embd[train_edge_index[0]].shape, x_embd[train_edge_index[1]].shape)
        ...
        with torch.no_grad():
            ...
            test_loss = np.sqrt(mse.item())
            l1 = F.l1_loss(pred_test, label_test)
            test_l1 = l1.item()
            if args.save_prediction:
                if epoch == best_valid_loss_epoch:
                    obj['outputs']['best_valid_loss_pred_test'] = pred_test.detach().cpu().numpy()
    ...
    if args.save_prediction and args.valid > 0.:
        plot_sample(obj['outputs'], log_path+'outputs_best_valid.png', 
                    groups=[['best_valid_loss_pred_test','label_test'],
