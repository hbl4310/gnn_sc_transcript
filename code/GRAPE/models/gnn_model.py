...
def get_gnn(data, args):
    ...
    model = GNNStack(data, 
                    ...
                    norm_embs, args.aggr, args.embedding_input_dim)
    return model
...
class GNNStack(torch.nn.Module):
    def __init__(self, 
        ...
        if is_data_homoegenous(data): 
            node_input_dim, edge_input_dim = data.num_node_features, data.edge_attr_dim
            self.het = False
            self.embedding = None
        else:
            edge_input_dim = set(v.shape[1] for v in data.collect('edge_attr').values())
            assert len(edge_input_dim) == 1, "Only supports uniform edge attr dim for now"
            edge_input_dim = list(edge_input_dim)[0]
            node_input_dim = max(v.shape[1] for v in data.collect('x').values())
            self.het = True
            self.embedding = GeneCellEmbedding(embedding_input_dim, node_input_dim)
...
class GeneCellEmbedding(torch.nn.Module):
    # input_dim: number of genes used, output_dim: embedding dimension
    def __init__(self, input_dim, output_dim): 
        super(GeneCellEmbedding, self).__init__()
        self.gene_embedding = torch.nn.Embedding(input_dim, output_dim)

    # expect x,  from HeteroData.collect
    # output combined x 
    def forward(self, x):
        gene_emb = self.gene_embedding(x['gene'].int()).squeeze()
        return torch.cat([x['cell'], gene_emb])


def is_data_homoegenous(data):
